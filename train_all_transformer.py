...
mel_text_stop_dataset = mel_text_stop_dataset.shuffle(10000).padded_batch(4, padded_shapes=([-1, 80], [-1], [-1]))
    ...
    for (batch, (mel, text, stop)) in enumerate(mel_text_stop_dataset):
        ...
        if batch_count % 200 == 199:
            ...
            for i in range(10, 13):
                test_mel = mel_text_stop_samples[i][0]
                test_sentence = tokenizer.encode(mel_text_stop_samples[i][1])
                ...
                print('(target) {}\n(mel to text) {}\n(text to text) {}\n'.format(
                    mel_text_stop_samples[i][1], pred, text_pred))
            print('\n>>> Validation samples')
            for i in range(3):
                test_mel = mel_text_stop_samples[i][0]
                test_sentence = tokenizer.encode(mel_text_stop_samples[i][1])
