...
class TextTransformer(tf.keras.Model):
    ...
    def __init__(self, num_layers, d_model, num_heads, dff, pe_input, pe_target, vocab_size: dict, rate=0.1):
        ...
        self.encoder = Encoder(num_layers, d_model, num_heads, dff, pe_input,
                               prenet=tf.keras.layers.Embedding(vocab_size['in'], d_model), rate=rate)
        self.decoder = Decoder(num_layers, d_model, num_heads, dff, pe_target,
                               prenet=tf.keras.layers.Embedding(vocab_size['out'], d_model), rate=rate)
...
class MelTransformer(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.encoder = Encoder(num_layers, d_model, num_heads, dff, pe_input,
                               prenet=PointWiseFFN(d_model=d_model, dff=dff), rate=rate)
        self.decoder = Decoder(num_layers, d_model, num_heads, dff, pe_target,
                               prenet=PointWiseFFN(d_model=d_model, dff=dff), rate=rate)
        self.out_module = SpeechOutModule(mel_channels=mel_channels,
    ...
    def train_step(self, inp, tar, stop_prob):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = weighted_sum_losses((tar_real, tar_stop_prob),
                                       (predictions, stop_prob),
                                       self.loss_list,
                                       self.loss_coeffs)
            #loss = masked_loss_function(tar_real, predictions, loss_object=self.loss_object)
    ...
    def predict(self, encoded_inp_sentence, MAX_LENGTH=40, stop_tolerance=0.5):
        ...
        out_dict = {'output': tf.squeeze(output, axis=0),
                    'attn_weights': attention_weights, 'logits': predictions, 'stop_probs': tf.squeeze(stop_probs)}
