...
train_dataset = train_dataset.shuffle(1000).padded_batch(4, padded_shapes=([-1, 80], [-1]))
...
for epoch in range(10000):
    for (batch, (inp, tar)) in enumerate(train_dataset):
        ...
        pred = mel_text_transformer.predict(train_samples[0][0])
        pred = tokenizer.decode(pred['output'])
        print('\nbatch count: {}, mean loss: {}\n(target) {}\n(pred) {}'.format(
            batch_count, sum(losses)/len(losses), train_samples[0][1], pred))
        batch_count += 1